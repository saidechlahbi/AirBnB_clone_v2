...
class TestAdditionalAmenityMethods(unittest.TestCase):
    """Additional test functions for Amenity class"""

    def test_init_with_name(self):
        """Test initialization of Amenity with a name"""
        amenity = Amenity(name="Swimming Pool")
        self.assertEqual(amenity.name, "Swimming Pool")

    def test_init_without_name(self):
        """Test initialization of Amenity without providing a name"""
        amenity = Amenity()
        self.assertEqual(amenity.name, "")

    def test_attribute_modification(self):
        """Test modification of Amenity attributes"""
        amenity = Amenity()
        amenity.name = "Tennis Court"
        self.assertEqual(amenity.name, "Tennis Court")

    def test_deletion(self):
        """Test deletion of Amenity instance"""
        amenity = Amenity()
        amenity_id = amenity.id
        amenity.delete()
        self.assertIsNone(Amenity.get(amenity_id))

    def test_serialization_with_additional_attrs(self):
        """Test serialization with additional attributes"""
        amenity = Amenity(name="Gym", category="Fitness")
        amenity_dict = amenity.to_dict()
        self.assertEqual(amenity_dict['category'], "Fitness")

    def test_str_with_additional_attrs(self):
        """Test string representation with additional attributes"""
        amenity = Amenity(name="Sauna", rating=5)
        str_output = "[Amenity] ({}) {}".format(amenity.id, amenity.__dict__)
        self.assertIn("rating", str_output)

    def test_update(self):
        """Test update of Amenity attributes"""
        amenity = Amenity(name="Spa")
        amenity.update(name="Massage Center")
        self.assertEqual(amenity.name, "Massage Center")
